...
func CreateTarArchive(dir string, target string, targetDiskSize int64, imageDist string) error {
	...
	//ovf := filepath.Join(dir, imageDist+"-"+VolNameRaw)
	ovf := filepath.Join(dir, fmt.Sprintf("%s-%s", imageDist, VolNameRaw))
	...
	//ovfSpec, err := Render(filepath.Base(target), imageDist+"-"+VolNameRaw, volSize, targetDiskSize, imageDist)
	ovfSpec, err := Render(filepath.Base(target), fmt.Sprintf("%s-%s", imageDist, VolNameRaw), volSize, targetDiskSize, imageDist)
	if err != nil {
